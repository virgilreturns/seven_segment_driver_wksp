...
int app_main(){
	...
	while(1){
		//SEVSEG_DigitTx(&sevseg);
			...
			enum ENUM_SEVSEG_CHAR TEMP = SEVSEG_CHAR_ARRAY[sevseg.digit_select[sevseg.refresh_target].current_char_index]; // debugging
			HAL_SPI_Transmit(&hspi2, &(SEVSEG_CHAR_ARRAY[sevseg.digit_select[sevseg.refresh_target].current_char_index]),1,1000); // spi transmit

			HAL_GPIO_WritePin(SPI_LATCH_GPIO_Port, SPI_LATCH_Pin, GPIO_PIN_SET); // latch pin pulse
